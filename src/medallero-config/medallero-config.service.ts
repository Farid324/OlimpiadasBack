...
export class MedalleroConfigService {
  ...
  async findAll(): Promise<MedalleroConfigWithDetails[]> {
    ...
    this.logger.debug(`Encontradas ${inscripcionesAgrupadas.length} combinaciones Area/Nivel con participantes`);
    ...
    for (const grupo of inscripcionesAgrupadas) {
        ...
        const area = await this.prisma.areas.findUnique({ where: { id_area }, select: { nombre_area: true } });
        const nivel = await this.prisma.niveles.findUnique({ where: { id_nivel }, select: { nombre_nivel: true } });

        // ✨ AJUSTE para manejar áreas/niveles eliminados o no encontrados
        const areaNombre = area?.nombre_area ?? `[Área No Encontrada ID: ${id_area}]`;
        const nivelNombre = nivel?.nombre_nivel ?? `[Nivel No Encontrado ID: ${id_nivel}]`;

        // Opcional: Si quieres omitir las inscripciones con áreas/niveles eliminados, usa:
  ...
  async create(dto: CreateMedalleroConfigDto) {
    this.logger.debug(`Creando medallero para area ${dto.id_area} y nivel ${dto.id_nivel}`);
    
    // Opcional: Verificar si ya existe una configuración para esta Area/Nivel (la clave única de Prisma también lo haría)
    /*
    const existing = await this.prisma.medallero_config.findUnique({
        where: { uq_medallero_area_nivel: { id_area: dto.id_area, id_nivel: dto.id_nivel } }
    });
    if (existing) {
        throw new ConflictException('Ya existe una configuración de medallero para esta área y nivel.');
    }
    */
  ...
  async update(id: number, dto: UpdateMedalleroConfigDto) {
    ...
    if (!id || id <= 0) {
      this.logger.debug(`ID de medallero es 0 o inválido. Creando nueva configuración para Area ${dto.id_area}/Nivel ${dto.id_nivel}...`);
      ...
      const createDto: CreateMedalleroConfigDto = {
       ...
       try {
           return await this.create(createDto); // Reutilizamos la función de creación
       } catch (e) {
            // Manejo de error si la creación falla (ej: clave única duplicada)
            // Esto solo debería ocurrir si el frontend mandó id_medallero=0 pero la configuración ya existe.
            this.logger.warn(`Intento de creación fallido para Area ${dto.id_area}/Nivel ${dto.id_nivel}. Buscando existente para actualizar...`);
    ...
    try {
        ...
        this.logger.error(`Error al actualizar medallero con ID ${id}: ${error.message}`);
        throw new NotFoundException(`Configuración de medallero con ID ${id} no encontrada.`);
    }
  }
