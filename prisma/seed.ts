...
async function main() {
  ...
  // ============================================================
  // CONFIG: MEDALLERO + CIERRES DE FASE + EVALUACIONES + PREMIOS
  // ============================================================

  // 1) Medallero por área + nivel
  await prisma.medallero_config.createMany({
    data: [
      // Matemática / Secundaria
      {
        id_area: areaMate.id_area,
  ...
  // 2) Cierres de FASE FINAL validados (para habilitar reportes/certificados)
  const adminUser = await prisma.usuarios.findFirst({
    where: { correo: emailAdmin },
  });
  if (!adminUser) {
    throw new Error('Admin no encontrado al crear cierres de fase.');
  }

  await prisma.cierres_fase.createMany({
    data: [
      {
        id_fase: faseFinal.id_fase,
        id_area: areaMate.id_area,
        id_nivel: secundaria.id_nivel,
        cerrado_por: responsableMath.id_usuario,
        estado_validacion: 'VALIDADO',
        validado_por: adminUser.id_usuario,
      },
      {
        id_fase: faseFinal.id_fase,
        id_area: areaMate.id_area,
      ...
      },
      {
        ...
        id_area: areaFisica.id_area,
        id_nivel: secundaria.id_nivel,
        cerrado_por: responsablePhys.id_usuario,
        estado_validacion: 'VALIDADO',
        validado_por: adminUser.id_usuario,
      },
    ],
  ...
  // 3) Evaluaciones de FASE FINAL, todas FIRMADAS (para notas finales)
  const inscFinales = await prisma.inscripciones.findMany({
    where: {
      OR: [
        { id_area: areaMate.id_area, id_nivel: secundaria.id_nivel },
        { id_area: areaMate.id_area, id_nivel: primaria.id_nivel },
        { id_area: areaFisica.id_area, id_nivel: secundaria.id_nivel },
      ],
    },
    select: { id_inscripcion: true },
  });

  // Asignamos notas diferentes pero simples (70, 73, 76, ...)
  await prisma.evaluaciones.createMany({
    data: inscFinales.map((insc, idx) => ({
      id_inscripcion: insc.id_inscripcion,
      id_fase: faseFinal.id_fase,
      id_evaluador: evaluador.id_usuario,
      nota: 70 + idx * 3, // number, Prisma lo convierte a Decimal
      estado_registro: 'FIRMADA',
      comentario: 'Seed evaluación FINAL (firmada)',
    })),
    skipDuplicates: true,
  });

  // 4) Premios otorgados según medallero
  const anioActual = new Date().getFullYear();
  ...
  const getInscIdSeed = (ci: string, idArea: number, idNivel: number) => {
    const insc = inscByCiAreaNivel.find(
      (i) =>
        i.competidor?.ci === ci &&
        i.id_area === idArea &&
  ...
  // Tipamos explícitamente: esto obliga a que 'tipo' sea tipo_premio y 'fuente' sea fuente_lista
  const premiosData: Prisma.premios_otorgadosCreateManyInput[] = [
    // Matemática / Secundaria
    {
      id_inscripcion: getInscIdSeed(
        'CI0001',
        areaMate.id_area,
        secundaria.id_nivel,
      ),
  ...
  // Opcional: marcar esas inscripciones como PREMIADO
  await prisma.inscripciones.updateMany({
    where: { id_inscripcion: { in: premiosData.map((p) => p.id_inscripcion) } },
    data: { estado_inscripcion: 'PREMIADO' },
  });

  // 5) Ajustar algunos competidores para que los reportes tengan datos más realistas
  const patchCompetidores = [
    {
      ci: 'CI0001',
      departamento: 'Cochabamba',
  ...
  console.log(
    '🏅 Premios de prueba creados para Ceremonia:',
    premiosData.length,
  );
  console.log(
    '📚 Departamentos y unidades educativas actualizados para:',
    patchCompetidores.length,
  );
